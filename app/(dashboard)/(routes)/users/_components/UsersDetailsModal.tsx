...
export default function UserDetailsModal({ isOpen, onClose, user }: UserDetailsModalProps) {
  ...
  const handleExportDetails = async () => {
    try {
      const response = await fetch(`/api/export-user?userId=${user.id}`, {
        method: "GET",
        headers: {
          ...
          <div className="h-full p-6 overflow-y-auto">
            ...
            <div className="flex justify-between border-b border-[#F8F8F8] dark:border-[#2A2A2A] py-3">
              <span className="text-red-500 font-medium text-sm">
                User ID: <span className="text-primary text-sm font-light">{user.id}</span>
              </span>
            </div>
            {/* Summary Section */}
              ...
              <div className="flex items-center justify-between space-x-1 pb-5 border-b border-[#F8F8F8] dark:border-[#2A2A2A]">
                <div className="flex items-center gap-2">
                  <Avatar className="w-13 h-13">
                    <AvatarImage src={user.logoUrl || "/placeholder-avatar.jpg"} alt={user.fullName} />
