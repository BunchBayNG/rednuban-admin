...
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = "https://redcollection.onrender.com/api/v1/analytics/payouts/count-summary";
    ...
    if (res.ok && data.status) {
      if (
        ...
        typeof data.data.success !== "number" ||
        typeof data.data.pending !== "number" ||
        typeof data.data.failed !== "number"
      ) {
        console.error("Invalid response data types:", data.data);
        return NextResponse.json(
          {
            statusCode: 500,
            status: false,
            message: "Invalid response data types from external API",
            data: null,
          },
          { status: 500 }
        );
